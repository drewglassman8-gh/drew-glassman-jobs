...
// localStorage adapter that mimics the storage API shape
const storage = {
  get: async (key) => {
    ...
    (async () => {
      try {
        const [a, s, p, c] = await Promise.all([
          storage.get("js:apps").catch(() => null),
          storage.get("js:saved").catch(() => null),
          storage.get("js:prep").catch(() => null),
          storage.get("js:contact").catch(() => null),
        ]);
